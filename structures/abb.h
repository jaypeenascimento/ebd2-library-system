...
static int abb_insert(ABB **root,
                      const char *name); // insere uma nova categoria na abb

int abb_load_categories(
    ABB **t,
    const char *path); // carrega categorias de um arquivo CSV para a abb

static void abb_list_categories(
    ABB *t); // lista todas as categorias presentes na abb em ordem alfabética
