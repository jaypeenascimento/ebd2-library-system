...
static ABB *node_create(const char *cat, const char *filename) {
    ...
    if (!n->category || !n->filename) { free(n->category); free(n->filename); free(n); return NULL; }
    n->left = n->right = NULL;
    return n;
}
...
int abb_insert(ABB **root, const char *name, const char *filename) {
    if (!root || !name || !filename) return -1;
    if (!*root) {
    ...
    while (1) {
        ...
        if (cmp < 0) {
            if (!cur->left) { cur->left = node_create(name, filename); return cur->left ? 0 : -1; }
            cur = cur->left;
        } else {
            if (!cur->right) { cur->right = node_create(name, filename); return cur->right ? 0 : -1; }
            cur = cur->right;
        }
    }
}
    ...
    while (1) {
        ...
        if (cols >= 3) {
            ...
            if (name && name[0] != '\0' && file && file[0] != '\0') {
                if (abb_insert(root, name, file) == 0) inserted++;
            }
        }
